...
const Board = () => {
  ...
  const handleOnDragEnd = async (result) => {
    ...
    if (source.droppableId !== destination.droppableId) {
      ...
      const updatedSourceTasks = sourceItems.map((task, index) => ({
        ...task,
        order: index,
      }));
      const updatedDestinationTasks = destinationItems.map((task, index) => ({
        ...task,
        order: index,
      ...
      updatedSourceTasks.forEach(async (updatedTask) => {
        if (
          tasks.find((t) => t.id === updatedTask.id).order !==
            updatedTask.order ||
          tasks.find((t) => t.id === updatedTask.id).flowNodeId !==
            updatedTask.flowNodeId
        ) {
      ...
      const updatedTasks = items.map((task, index) => ({
        ...task,
        order: index,
        flowNodeId: Number(source.droppableId),
      }));

      updatedTasks.forEach(async (updatedTask) => {
        if (
          tasks.find((t) => t.id === updatedTask.id).order !== updatedTask.order
        ) {
          dispatch(updateTask(updatedTask));
          dispatch(fetchTasks(Number(source.droppableId)));
        }
